...
final class NSManagedObjectContextObserversTests: CoreDataPlusInMemoryTestCase {
  ...
  func testMerge() throws {
    ...
    // [4] observer
    let cancellable1 = NotificationCenter.default.publisher(for: .NSManagedObjectContextWillSave, object: backgroundContext)
      .sink { _ in
        expectation1.fulfill()
    }

    // [1] observer
      let cancellable2 = NotificationCenter.default.publisher(for: .NSManagedObjectContextDidSave, object: backgroundContext)
        .map { Payload.NSManagedObjectContextDidSave(notification: $0) }
        .sink { payload in
      ...
      viewContext.performAndWaitMergeChanges(from: payload) // fires [2]
    ...
    // [0] observer
    let cancellable3 = NotificationCenter.default.publisher(for: .NSManagedObjectContextObjectsDidChange, object: backgroundContext)
    ...
    // [2] observer
    let cancellable4 = NotificationCenter.default.publisher(for: .NSManagedObjectContextObjectsDidChange, object: viewContext)
    .map { Payload.NSManagedObjectContextObjectsDidChange(notification: $0) }
    .sink { payload in
    ...
    // [3] observer
      let cancellable5 = NotificationCenter.default.publisher(for: .NSManagedObjectContextDidSave, object: viewContext)
          .map { Payload.NSManagedObjectContextDidSave(notification: $0) }
          .sink { payload in
