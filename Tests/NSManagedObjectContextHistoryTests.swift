...
final class NSManagedObjectContextHistoryTests: XCTestCase {
  ...
  func testInvestigationPersistentStoreRemoteChangeAndBatchOperations() throws {
    ...
    let cancellable1 = NotificationCenter.default.publisher(for: Notification.Name.NSPersistentStoreRemoteChange, object: container1.persistentStoreCoordinator).sink { (notification) in
      // TODO Payload
      print(notification)
      expectation1.fulfill()
    }
    ...
    let cancellable2 = NotificationCenter.default.publisher(for: Notification.Name.NSPersistentStoreRemoteChange, object: container2.persistentStoreCoordinator).sink { (notification) in
      // TODO Payload
      print(notification)
      expectation2.fulfill()
    }
