...
public enum SampleModelVersion3: String, CaseIterable {
...
extension SampleModelVersion3: ModelVersion {
  ...
  public var successor: SampleModelVersion3? { // TODO: rename as next
    switch self {
...
extension SampleModelVersion3 {
  public func mappingModelsToNextModelVersion() -> [NSMappingModel]? { nil }
}
  ...
  public func migrationStagesToNextModelVersion() -> [NSMigrationStage]? { // TODO return a single stage
    ...
    case .version1:
      let stage = NSCustomMigrationStage(migratingFrom: self.managedObjectModelReference(), // v1
                                         to: self.successor!.managedObjectModelReference()) // v2
      ...
      return [stage]
    case .version2:
      let stage = NSLightweightMigrationStage([self.successor!.versionChecksum]) // v3
...
// TODO: Write why migraton isn't working with programmatic
